...
describe('AIProvider Interface Compliance', () => {
  ...
  describe('Configuration Handling', () => {
    ...
    it('should handle configuration loading errors', async () => {
      ...
      const azureProvider = new AzureOpenAIProvider(errorConfigManager as unknown as ConfigManager);
      
      // These methods are synchronous and don't call config, but generateText should fail
      expect(azureProvider.getDefaultModel()).toBe('o3');
      expect(azureProvider.listModels()).toEqual(['o3']);
      
      // Test async methods that do call config
      await expect(azureProvider.generateText({ prompt: 'test', temperature: 0.7, useSearchGrounding: false })).rejects.toThrow('Config loading failed');
    });
  });
